...
fnames = ['gru_hard', 'lstm_hard' , 'gru_learned', 'lstm_learned']
ftypes = ['ng_longer'] #, 'ng_unseen', 'ng_unseen_longer']
...
for fname in final_fnames:
    try:
        ...
        df = pd.read_csv(os.path.join(opt.logfolder,'New_AG',file_folder, fname), delim_whitespace=True)
        df = df.T
        ...
        for d in dtypes:
            ...
            if 'learned' in fname.split('_'):
                old_cols = new_df.columns.values.tolist()[idx+1:idx+6]
            else:
        ...
        plt.savefig(os.path.join(opt.plotfolder,'New_AG', fname+'_accuracies.png'))
