...
fnames = ['gru_baseline', 'lstm_baseline']
...
for fname in final_fnames:
    try:
        ...
        df = pd.read_csv(os.path.join(opt.logfolder,'Ponderless',file_folder, fname), delim_whitespace=True)
        df = df.T
        ...
        for d in dtypes:
            ...
            old_cols = new_df.columns.values.tolist()[idx+1:idx+4]
        ...
        plt.savefig(os.path.join(opt.plotfolder,'Ponderless', fname+'_accuracies.png'))
