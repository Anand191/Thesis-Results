...
fnames = ['random_hard', 'random_baseline']
ftypes = ['unseen']
...
for fname in final_fnames:
    ...
    accuracies = [l for l in new_df.columns.values.tolist() if 'acc' in l.split('_') and 'erm' not in l.split('_')]

    plt.figure()
    new_df.plot(x='steps', y = accuracies)
    plt.title('Accuracies_{}'.format(fname))
    #plt.savefig(os.path.join(opt.plotfolder,'Hard', fname+'_accuracies.png'))
    plt.show()
